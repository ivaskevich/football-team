...
import com.football.team.entity.dto.CaptchaResponseDto;
...
public class RegistrationController {
    ...
    public String addUser(@RequestParam("passwordConfirm") String passwordConfirm,
        ...
        if (response != null && !response.isSuccess()) {
            model.addAttribute("captchaError", "Пройдіть перевірку на спам!");
        }

        boolean confirmIsEmpty = passwordConfirm == null || passwordConfirm.equals("");

        if (confirmIsEmpty) {
            model.addAttribute("passwordConfirmError", "Повторний пароль не може бути порожнім!");
        }
        ...
        if (confirmIsEmpty || bindingResult.hasErrors() || response == null || !response.isSuccess()) {
        ...
        model.addAttribute("message", "Check your email , please and follow instructions!");
        return "login";
    }
    ...
    public String activate(Model model, @PathVariable String code) {
        ...
        if (isActivated) {
            ...
            model.addAttribute("message", "Activation code if not found!");
